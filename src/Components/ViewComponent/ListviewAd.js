...
const ListviewAd = ({ rowsPerPage, page }) => {
  ...
  const fetchPost = async () => {
    try {
      let res = await axios.get(`${process.env.REACT_APP_URL}/bigyapan/break`);
      console.log(res.data.ads, "res.data")
  ...
  const handleDelete = async (id) => {
    await axios
      .delete(`${process.env.REACT_APP_URL}/deleteAd/${id}`, {headers: {
            ...
            {data?.map((item, index) => (
              ...
              >
                ...
                >
                  <div className="flex items-center gap-5 hover:text-primary-blue cursor-pointer">
                    <img
                       className="w-[400px] h-[100px] object-cover rounded-lg  "
                       src={item.image}
                      alt=""
                    />{" "}
                    {/* {item.image} */}
